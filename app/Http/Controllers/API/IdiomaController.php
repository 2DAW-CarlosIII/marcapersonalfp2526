    ...
    public function index(Request $request, $userId = null)
    {
        if ($userId !== null) {
            return IdiomaResource::collection(
                User::findOrFail($userId)->idiomas()->orderBy($request->sort ?? 'id', $request->order ?? 'asc')->paginate($request->per_page)
            );
        }

        return IdiomaResource::collection(
            Idioma::orderBy($request->sort ?? 'id', $request->order ?? 'asc')->paginate($request->per_page)
        );
    }
